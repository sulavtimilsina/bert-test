\# %% [code] {"jupyter":{"outputs_hidden":true},"execution":{"iopub.status.busy":"2022-02-17T03:54:06.512548Z","iopub.execute_input":"2022-02-17T03:54:06.513050Z","iopub.status.idle":"2022-02-17T03:54:14.973690Z","shell.execute_reply.started":"2022-02-17T03:54:06.512993Z","shell.execute_reply":"2022-02-17T03:54:14.972233Z"}}
import pickle
import pandas as pd
import numpy as np
...
# %% [code] {"jupyter":{"outputs_hidden":false},"execution":{"iopub.status.busy":"2022-02-17T03:38:51.353476Z","iopub.execute_input":"2022-02-17T03:38:51.353778Z","iopub.status.idle":"2022-02-17T03:38:51.374814Z","shell.execute_reply.started":"2022-02-17T03:38:51.353748Z","shell.execute_reply":"2022-02-17T03:38:51.373855Z"}}
...
# %% [code] {"jupyter":{"outputs_hidden":false},"execution":{"iopub.status.busy":"2022-02-17T03:51:36.606221Z","iopub.execute_input":"2022-02-17T03:51:36.606914Z","iopub.status.idle":"2022-02-17T03:51:36.680825Z","shell.execute_reply.started":"2022-02-17T03:51:36.606871Z","shell.execute_reply":"2022-02-17T03:51:36.679096Z"}}
class creating_pretraining_data:

    def __init__(self, path_to_text, path_to_vocab, path_to_output,path_to_restore,load_previous=True , masking_per=0.15, sentence_len=512):
        ...
        self.path_to_restore = path_to_restore

        if load_previous==True:
            #Reading the previously number of  completed recoreds
            a_file = open(self.path_to_restore, "rb")
            self.restore_dictionary = pickle.load(a_file)
            a_file.close()

        else:
            a_file = open(self.path_to_restore, "wb")
            pickle.dump({'retrive':0},a_file)
            a_file.close()
            self.restore_dictionary= {'retrive':0}
    ...
    def reading_file(self, df):
        ...
        print('The number of text already tokenized are:',self.restore_dictionary['retrive'])

        for text in tqdm(df[self.restore_dictionary['retrive']:]):
            restore += 1
            self.restore_dictionary= {'retrive':restore}
            a_file = open(self.path_to_restore, "wb")
            pickle.dump(self.restore_dictionary, a_file)
            a_file.close()
...
# %% [code] {"jupyter":{"outputs_hidden":false},"execution":{"iopub.status.busy":"2022-02-17T03:51:55.537612Z","iopub.execute_input":"2022-02-17T03:51:55.537891Z","iopub.status.idle":"2022-02-17T03:51:55.611234Z","shell.execute_reply.started":"2022-02-17T03:51:55.537861Z","shell.execute_reply":"2022-02-17T03:51:55.610008Z"}}
path_to_vocab = '../input/nepalivocab/nepali-vocab.txt'
path_to_output = './nepali.tfrecords'
path_to_restore = './restore.pkl'
percentage = 0.15
sen_len = 512


preprocessor = creating_pretraining_data(path_to_text, path_to_vocab, path_to_output,path_to_restore,True, percentage, sen_len)
preprocessor.reading_file(df.text)

# %% [code] {"jupyter":{"outputs_hidden":false},"execution":{"iopub.status.busy":"2022-02-16T17:11:24.858712Z","iopub.execute_input":"2022-02-16T17:11:24.859463Z","iopub.status.idle":"2022-02-16T17:11:24.955618Z","shell.execute_reply.started":"2022-02-16T17:11:24.859423Z","shell.execute_reply":"2022-02-16T17:11:24.954400Z"}}
# # path_to_vocab= '../input/nepalivocab/nepali-vocab.txt'
